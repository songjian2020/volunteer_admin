...
export default function XinTable<T extends Record<string, any> = any>(props: XinTableProps<T>) {
  ...
  const handleCreate = useCallback(async () => {
    ...
    let initials: Partial<T> = {};
    if (createInitialValues) {
      initials = typeof createInitialValues === 'function'
        ? await createInitialValues()
        : createInitialValues;
    }
    if (initials && Object.keys(initials).length > 0) {
      formRef.current?.setFieldsValue(initials as T);
    }
